<?php include 'check.php';  $pageurl="addfeatures"; $tbl_name="features"; $sessionid=date("ymdHis"); $pagename=$_GET['pagename'];


if(isset($_GET['mode']) && $_GET['mode'] == "create_new"){
    $features_title = $con->real_escape_string($_POST['features_title']);
    $features_description = $con->real_escape_string($_POST['features_description']);
    ...
    if (!empty($tbl_name)) {
        $sql = "INSERT INTO `$tbl_name`(`is_active`, `features_title`,`features_description`, `is_deleted`, `created_at`) 
                VALUES ('1', '$features_title','$features_description', 'Live', '$created_at')";
    ...
    $sql = "UPDATE `$tbl_name` SET features_title='$features_title', features_description='$features_description' WHERE id ='".$_GET['id']."'";
    ...
    <form name="form1" id="form1" action="<?php echo $pageurl ?>?mode=<?php echo isset($_GET['id']) ? 'update' : 'create_new' ?>&pagename=<?php echo $pagename ?>&id=<?php echo @$_GET['id'] ?>&tbl_name=<?php echo $tbl_name ?>" method="post">
        <div align="center">
            <icon class="btn bg-white margin">
                <label> Features Title<span style="color:#FF0000">*</span></label>
                <input type="text" name="features_title" value="<?php echo $features_title ?>" class="form-control" required="required" style="width:200px;" />
            </icon>	
            <!-- <div style="height:15px;"></div> -->
            <icon class="btn bg-white margin">
                <label>Description<span style="color:#FF0000"></span></label>
                <input type="text" name="features_description" value="<?php echo $features_description ?>" class="form-control" style="width:200px;" />
            </icon>	<br/>
            <!-- <div style="height:15px;"></div> -->
