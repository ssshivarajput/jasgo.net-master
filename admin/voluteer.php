<?php include 'check.php'; $pageurl="volunteer"; $tbl_name="volunteer"; $mintablewidth="950px";  $utype="volunteer"; $pagename="$utype Details";
...
<table width="100%" border="0">
  <tr>
    <td><a onClick="popupCenter('addstudent<?php echo $extn ?>', 'myPop1',520,600);" href="javascript:void(0);" style="color:#0000FF; text-transform:uppercase"><button type="button" class="btn btn-success btn-sm"><i class='fa fa-plus-square'></i> CREATE<span class="hidem"> NEW <?php echo strtoupper($utype) ?></span></button></a> </td>
...
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name`"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col --></a>
...
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_active='1'"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col --></a>
...
/*	Setup vars for query. */
	...
	$tnum = $total_pages;} else {$tnum = $total_pages-($limit*($page-1));}
	if($page) 
...
echo "<td align='left' style='padding-left:10px;'>Email</td>";
echo "<td align='left' style='padding-left:10px;'>Mobile Number</td>";
echo "<td align='left' style='padding-left:10px;'>Address</td>";

echo "</tr>";

$in=0; while($row = mysqli_fetch_array($result)) {$rdx=date("d/m/Y", strtotime($row['schedule_date_time'])); 
...
echo "<td valign='top' style='padding:5px;'><a href='https://sanatanworld.org/assets/uploads/volunteer/$row[image_url]' target='_blank'><img src='https://sanatanworld.org/assets/uploads/volunteer/$row[image_url]' width='80' /></a><br /><div style='height:4px;'></div>$rdx</td>";
echo "<td align='left' style='padding:5px;line-height:20px;' valign='top'><b>$row[title]</b>"; ?> <a onClick="popupCenter('addvolunteer?id=<?php echo $row['id'] ?>', 'myPop1',520,600);" href="javascript:void(0);" style="text-decoration:none;color:#00F; cursor:pointer;"><img src='images/edit.png' title='Edit'></a><?php echo "<br />$row[description]</td>";
