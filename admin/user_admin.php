<?php include 'check.php'; $pageurl="user_admin"; $tbl_name="user_information"; $mintablewidth="950px";  $utype="user_admin"; $pagename="$utype Details";
...
<table width="100%" border="0">
  <tr>
    <td><a onClick="popupCenter('adduseradmin<?php echo $extn ?>', 'myPop1',400,350);" href="javascript:void(0);" style="color:#0000FF; text-transform:uppercase"><button type="button" class="btn btn-success btn-sm"><i class='fa fa-plus-square'></i> CREATE<span class="hidem"> NEW <?php echo strtoupper($utype) ?></span></button></a> </td>
...
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_active='1' and is_deleted='Live'and user_type='$utype'"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col --></a>
...
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_active='0' and is_deleted='Live'and user_type='$utype'"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col --></a>
			
<div class="col-lg-3 col-xs-6">
<!-- small box -->
<div class="small-box  bg-red-gradient"><div class="inner">Archive <?php echo $utype ?> : <b style="font-size:15px;">
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_deleted='Deleted'and user_type='$utype'"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col -->		   
</div>	
...
echo "<td align='left' style='padding-left:10px;'>User Name</td>";

echo "<td align='left'width:99;'>User Code</td>";
echo "<td align='left' style='padding-left:10px;'>User Email</td>";
...
echo "<td style='padding:5px;' valign='top'>"; if($row['is_active']=="1"){?><span style="color:#009900">Active</span><?php } else {?><span style="color:#FF0000">Inactive</span><?php } echo"</td>";
echo "<td align='left' style='padding:5px;line-height:20px;' valign='top'><b>$row[user_name]</b>"; ?><a onClick="popupCenter('adduseradmin?id=<?php echo $row['id'] ?>', 'myPop1',400,350);" href="javascript:void(0);" style="text-decoration:none;color:#00F; cursor:pointer;"><img src='images/edit.png' title='Edit'></a><?php echo "</td>";

echo "</td>";

echo "<td align='left' style='padding:5px;line-height:20px;' valign='top'>$row[user_code]</td>";
echo "</td>";

echo "<td align='left' style='padding:5px;line-height:20px;' valign='top'>$row[user_email]</td>";
