<?php include 'check.php'; $pageurl="unknown_user"; $tbl_name="users"; $mintablewidth="950px";  $utype="NULL"; $pagename="$utype Details";
...
<table width="100%" border="0">
  <tr>
    <td><a onClick="popupCenter('addvolunteer<?php echo $extn ?>', 'myPop1',520,400);" href="javascript:void(0);" style="color:#0000FF; text-transform:uppercase"><button type="button" class="btn btn-success btn-sm"><i class='fa fa-plus-square'></i> CREATE<span class="hidem"> NEW <?php echo strtoupper($utype) ?></span></button></a> </td>
...
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` where is_deleted='Live' and user_type IS NULL"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col --></a>
...
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_active='1' and is_deleted='Live' and user_type IS NULL"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col --></a>
...
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_active='0' and is_deleted='Live' and user_type IS NULL"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col --></a>
			
<div class="col-lg-3 col-xs-6">
<!-- small box -->
<div class="small-box  bg-red-gradient"><div class="inner">Archive <?php echo $utype ?> : <b style="font-size:15px;">
<?php $querym = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_deleted='Deleted' and user_type IS NULL"; $querymc = mysqli_fetch_array(mysqli_query($con,$querym)); $totalquery= $querymc['num']; echo $totalquery ?></b></div>
</div></div><!-- ./col -->		   
</div>	
...
<div style='overflow-x: auto;'>
	...
	*/
	...
	$query = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE is_deleted='Live' and user_type IS NULL";
    $sqlx = "SELECT * FROM `$tbl_name` WHERE is_deleted='Live' and user_type IS NULL"; 
...
echo "<td align='left' style='padding-left:10px;'>Image</td>";
echo "<td align='left' style='padding-left:10px;'>Volunteer name</td>";
echo "<td style='padding-left:10px;'>Email</td>";
echo "<td style='padding-left:10px;'>Mobile Number</td>";
echo "<td style='padding-left:10px;'>Address</td>";
...
echo "<td <div style='height:6px;' width: 10px;'valign='top'></div><input type='checkbox' class='td' name =ids[] value='$row[id]' style='cursor:pointer;'</td>";
...
echo "<td style='padding:5px;'>";?><a onClick="popupCenter('projectimage.php?folder=user&column=profile_pic&table=<?php echo $tbl_name ?>&w=500&h=500&id=<?php echo $row['id'] ?>', 'myPop1',450,500);" href="javascript:void(0);" style="text-decoration:none;color:#00F; cursor:pointer;"><img src="<?php echo $baseurl ?>/assets/uploads/user/<?php echo $row['profile_pic'] ?>" width="80" /></a><?php echo "</td>";




echo "<td align='left' style='padding:5px;line-height:20px; width:157px;' valign='top'><b>$row[full_name]</b>"; ?> <a onClick="popupCenter('addvolunteer?id=<?php echo $row['id'] ?>', 'myPop1',520,400);" href="javascript:void(0);" style="text-decoration:none;color:#00F; cursor:pointer;"><img src='images/edit.png' title='Edit'></a><?php echo "</td>";

echo "<td style='padding:5px;line-height:20px;' valign='top'>$row[email]</td>";
echo "<td style='padding:5px;line-height:20px; width:140px;' valign='top'>$row[mobile_no]</td>";
echo "</td>";
;
echo "<td align='left' style='padding:5px;line-height:20px;width:230px;' valign='top'>$row[street_address]</td>";
echo "</tr>";
$tnum=$tnum-1; 	 
}
