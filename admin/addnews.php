<?php include 'check.php';  $pageurl="addnews"; $tbl_name="news"; $sessionid=date("ymdHis"); $pagename=$_GET['pagename'];


if(isset($_GET['mode']) && $_GET['mode'] == "create_new"){
    $news_title = $con->real_escape_string($_POST['news_title']);
    $news_description = $con->real_escape_string($_POST['news_news_description']);
    ...
    if (!empty($tbl_name)) {
        $sql = "INSERT INTO `$tbl_name`(`is_active`, `news_title`,`news_description`, `is_deleted`, `created_at`) 
                VALUES ('1', '$news_title','$news_description', 'Live', '$created_at')";
    ...
    $news_title = $con->real_escape_string($_POST['news_title']);  
    $news_description = $con->real_escape_string($_POST['news_description']);
  
    $sql = "UPDATE `$tbl_name` SET news_title='$news_title', news_description='$news_description' WHERE id ='".$_GET['id']."'";
    ...
    <form name="form1" id="form1" action="<?php echo $pageurl ?>?mode=<?php echo isset($_GET['id']) ? 'update' : 'create_new' ?>&pagename=<?php echo $pagename ?>&id=<?php echo @$_GET['id'] ?>&tbl_name=<?php echo $tbl_name ?>" method="post">
        <div align="center">
            <icon class="btn bg-white margin">
                ...
                <input type="text" name="news_title" value="<?php echo $news_title ?>" class="form-control" required="required" style="width:200px;" />
            </icon>	
            <!-- <div style="height:15px;"></div> -->
            <icon class="btn bg-white margin">
                <label>Description<span style="color:#FF0000">*</span></label>
                <input type="text" name="news_description" value="<?php echo $news_description ?>" class="form-control" required="required" style="width:200px;" />
            </icon>	<br/>
            <!-- <div style="height:15px;"></div> -->
